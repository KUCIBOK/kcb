...
async function importGalleries(req, res, next) {
  try {
    ...
    if (!req.file) {
      logger.warn("Aucun fichier reçu dans req.file");
      return next(createError.badRequest("Aucun fichier fourni (champ 'file')."));
    }

    logger.info("Fichier reçu", { file: req.file });

    const filePath = path.resolve(req.file.path);
    const ext = path.extname(filePath).toLowerCase();
    logger.info("Fichier importé", { filePath, ext });
    ...
    logger.info("Nombre de lignes parsées", { count: rows.length });
    ...
    for (const [idx, raw] of rows.entries()) {
      try {
        ...
        if (existing) {
          logger.info(`[Row ${idx + 1}] Mise à jour de ${email}`);
