...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (s0 === 'cron' && s1 === 'expire-subscriptions')
      return await routeCronExpireSubscriptions(req, res)
    ...
    if (s0 === 'blog' && s1 === 'comment' && s2)
      return await routeBlogComment(req, res, s2, segments[3])
    ...
    if (s0 === 'blog' && s1 === 'published' && s2?.startsWith('user'))
      return await routeBlogByUser(req, res, s2.replace('user', ''), 'published')

    // ── /api/blog/draft/user:id ────────────────────────────────────────────────
    if (s0 === 'blog' && s1 === 'draft' && s2?.startsWith('user'))
      return await routeBlogByUser(req, res, s2.replace('user', ''), 'draft')

    // ── /api/blog/archived/user:id ─────────────────────────────────────────────
    if (s0 === 'blog' && s1 === 'archived' && s2?.startsWith('user'))
      return await routeBlogByUser(req, res, s2.replace('user', ''), 'archived')
    ...
    if ((s0 === 'categories' || s0 === 'category') && s1)
      return await routeCategoryById(req, res, s1)
    ...
    if (s0 === 'numerisation' && s1 && s2 === 'status')
      return await routeNumerisationStatus(req, res, s1)
    ...
    if (s0 === 'payments' && s1 === 'paydunya-callback')
      return await routePaydunyaCallback(req, res)

    // ── /api/transaction/fail/:id ──────────────────────────────────────────────
    if (s0 === 'transaction' && s1 === 'fail' && s2)
      return await routeTransactionStatus(req, res, s2, 'failed')

    // ── /api/transaction/ref/:ref ──────────────────────────────────────────────
    if (s0 === 'transaction' && s1 === 'ref' && s2)
      return await routeTransactionByRef(req, res, decodeURIComponent(s2))
    ...
    if (s0 === 'subscription' && s1 === 'fail' && s2)
      return await routeSubscriptionStatus(req, res, s2, 'failed')

    // ── /api/subscription/activate/:id ─────────────────────────────────────────
    if (s0 === 'subscription' && s1 === 'activate' && s2)
      return await routeSubscriptionStatus(req, res, s2, 'active')
    ...
    if (s0 === 'crm' && s1 === 'clients' && s2 && segments[3] === 'notes')
      return await routeCrmNotes(req, res, s2, segments[4])

    // ── /api/crm/clients/:id/interactions/:interactionId ───────────────────────
    if (s0 === 'crm' && s1 === 'clients' && s2 && segments[3] === 'interactions')
      return await routeCrmInteractions(req, res, s2, segments[4])
    ...
    if (s0 === 'contacts' && s1 === 'contacts' && s2 === 'stats')
      return await routeContactStats(req, res)

    // ── /api/contacts/contacts/import ──────────────────────────────────────────
    if (s0 === 'contacts' && s1 === 'contacts' && s2 === 'import')
      return await routeContactImport(req, res)

    // ── /api/contacts/contacts/:id/unsubscribe ─────────────────────────────────
    if (s0 === 'contacts' && s1 === 'contacts' && s2 && segments[3] === 'unsubscribe')
      return await routeContactUnsubscribe(req, res, s2)
    ...
    if (s0 === 'contacts' && s1 === 'lists' && s2 && segments[3] === 'rsvp')
      return await routeContactListRsvp(req, res, s2)
    ...
    if (s0 === 'entities' && s1 && s2 === 'members')
      return await routeEntityMembers(req, res, s1, segments[3])
    ...
    if (s0 === 'integrations' && s1 && s2 === 'sync')
      return await routeIntegrationSync(req, res, s1)
    ...
    if (s0 === 'professional-analytics' && s1 === 'realtime')
      return await routeProAnalyticsRealtime(req, res)

    // ── /api/professional-analytics ────────────────────────────────────────────
    if (s0 === 'professional-analytics') return await routeProAnalytics(req, res)

    // ── /api/campaigns/campaigns/:id/* ─────────────────────────────────────────
    if (s0 === 'campaigns' && s1 === 'campaigns')
      return await routeCampaignsCrud(req, res, s2, segments[3])
...
async function routeHealthPayment(req, res) {
  ...
  try {
    const testRes = await fetch(endpoint, {
      ...
      body: JSON.stringify({
        invoice: { total_amount: 1, description: 'test', currency: 'XOF' },
        store: { name: 'test' },
      }),
    })
    ...
    try {
      pdBody = JSON.parse(raw)
    } catch {
      pdBody = raw.slice(0, 200)
    }
  } catch (err) {
    pdBody = err.message
  }
...
async function routeContact(req, res) {
  ...
  if (!rateLimit(ip, 60_000, 5))
    return fail(res, 'Trop de requêtes. Réessayez dans une minute.', 429)
...
async function routeAuth(req, res, action) {
  ...
  switch (action) {
    ...
    case 'signin':
      return await authSignin(req, res)
    case 'signout':
      return await authSignout(req, res)
    case 'me':
      return await authMe(req, res)
    case 'forgot-password':
      return await authForgotPassword(req, res)
    case 'reset-password':
      return await authResetPassword(req, res)
    case 'change-password':
      return await authChangePassword(req, res)
    case 'google-callback':
      return await authGoogleCallback(req, res)
    case 'send-access':
      return await authSendAccess(req, res)
    case 'create-buyer':
      return await authCreateBuyer(req, res)
    case 'set-role':
      return await authSetRole(req, res)
    default:
      return fail(res, 'Action auth inconnue', 404)
  }
}
 ...
 * Rôles autorisés : 'artist', 'curator', 'advisor'.
 *
 * @param {import('@vercel/node').VercelRequest}  req
 * @param {import('@vercel/node').VercelResponse} res
 */
async function authSetRole(req, res) {
  ...
  const { role } = req.body ?? {}
  const ALLOWED_ROLES = ['artist', 'curator', 'advisor']
...
async function authDeleteById(req, res, id) {
  ...
  if (authResult.user.id === id)
    return fail(res, 'Vous ne pouvez pas supprimer votre propre compte', 403)
...
async function authSignup(req, res) {
  ...
  if (!['artist', 'curator', 'advisor'].includes(role)) {
    return fail(res, 'Rôle invalide. Valeurs acceptées : artist, curator, advisor')
  }
...
async function authChangePassword(req, res) {
  ...
  if (!password || password.length < 8)
    return fail(res, 'Nouveau mot de passe requis (min. 8 caractères)')
...
async function authCreateBuyer(req, res) {
  ...
  if (signInError)
    return fail(res, 'Compte créé mais connexion échouée. Veuillez vous connecter manuellement.')
...
async function authSendAccess(req, res) {
  ...
  if (!Array.isArray(users) || !users.length)
    return fail(res, 'users doit être un tableau non vide')
  ...
  for (const u of users) {
    ...
    if (!email || !EMAIL_RE.test(email)) {
      results.push({
        email: email ?? null,
        status: 'skipped',
        reason: 'email manquant ou invalide',
      })
      continue
    }
    ...
    try {
      ...
      if (linkError) {
        results.push({ email, status: 'error', reason: linkError.message })
        continue
      }
      ...
      if (!recoveryLink) {
        results.push({
          email,
          status: 'error',
          reason: "Impossible de générer le lien d'accès (compte inexistant ?)",
        })
        continue
      }
...
async function routeArtworkById(req, res, id) {
  if (req.method === 'GET') {
    ...
    return ok(res, {
      ...data,
      _id: data.id,
      artist: data.artists?.name ?? data.artist ?? null,
      image,
    })
  }
...
async function routeArtists(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const { name, username, image, country, biography, portfolio, facebook, twitter, instagram } =
      req.body ?? {}
        ...
        await supabaseAdmin
          ...
          .then(({ error }) => {
            if (error) {
              supabaseAdmin
                .from('artists')
                .update({ visited: (data.visited ?? 0) + 1 })
                .eq('id', data.id)
            }
          })
      } catch {
...
async function routeDelivery(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (Array.isArray(artwork_ids) && artwork_ids.length > 0) {
      ...
      try {
        await supabaseAdmin.from('delivery_artwork_ids').insert(links)
      } catch {}
    }
...
async function routeGalleries(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const { data, error, count } = await query
    if (error) return fail(res, error.message)
    return ok(res, { galleries: data ?? [], total: count ?? 0, filtered: data?.length ?? 0 }, 200, {
      page,
      limit,
      total: count,
    })
...
async function routeGalleriesImport(req, res) {
  ...
  const rows = galleries.map((g) => ({
    name: String(g.name ?? '')
      .trim()
      .slice(0, FIELD_MAX.name),
    description: g.description
      ? String(g.description).trim().slice(0, FIELD_MAX.description)
      : null,
...
async function routeVisitor(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    return ok(
      res,
      (data ?? []).map((v) => ({ ...v, _id: v.id, createdAt: v.created_at, visitTime: 0 }))
    )
  }
...
async function routeSubscription(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (!txRecord)
      return fail(
        res,
        "Paiement non vérifié. Effectuez le paiement avant d'activer l'abonnement.",
        402
      )
...
async function generateCertificateForArtwork(artworkId) {
  ...
  const pdfBuffer = await new Promise((resolve, reject) => {
    ...
    const rows = [
      ...
      [
        'Dimensions',
        artwork.height && artwork.width ? `${artwork.height} × ${artwork.width} cm` : 'N/A',
      ],
    ...
    rows.forEach(([label, value], i) => {
      ...
      doc
        ...
        .font('Helvetica')
        .fillColor(GOLD)
        .text(label, lx + 10, ry + 9, { width: 120 })
      doc
        .fontSize(10)
        .font('Helvetica-Bold')
        .fillColor(DARK)
        .text(value, lx + 140, ry + 9, { width: tableW - 148, align: 'right' })
    })
    ...
    doc
      .rect(imgX - 2, ry2 - 2, imgSz + 4, imgSz + 4)
      .lineWidth(2)
      .stroke(GOLD)
    if (imageBuffer) {
      try {
        doc.image(imageBuffer, imgX, ry2, { width: imgSz, height: imgSz, cover: [imgSz, imgSz] })
      } catch (_) {
    ...
    doc
      ...
      .font('Helvetica')
      .fillColor(GRAY)
      .text("Ce certificat atteste de l'authenticité\nde cette œuvre d'art africaine.", rx, ry2, {
        width: rw,
        align: 'center',
      })
...
async function routeCertificateUrl(req, res, artworkId) {
  ...
  if (signErr || !signedData?.signedUrl)
    return fail(res, 'Impossible de générer le lien de téléchargement')
...
async function routeProfile(req, res, id) {
  ...
  if (req.method === 'PUT') {
    ...
    if (req.body?.telephone !== undefined) {
      await supabaseAdmin.from('users').update({ telephone: req.body.telephone }).eq('id', id)
    }
...
const KCB_COMMISSION_RATE = 0.1 // 10 % de commission Kucibok
...
async function routeTransactionById(req, res, id) {
  ...
  let query = supabaseAdmin
    .from('transactions')
    .select(
      '*, artworks(title, image, kucibok_id, price, currency), buyer:buyer_id(name, email), seller:seller_id(name, email)'
    )
    .eq('id', id)
...
async function routeTransactionStatus(req, res, id, status) {
  ...
  let query = supabaseAdmin.from('transactions').update({ status }).eq('id', id)

  if (role !== 'admin') query = query.or(`buyer_id.eq.${userId},seller_id.eq.${userId}`)

  const { data, error } = await query.select('*, artworks(title, image, kucibok_id)').single()
...
async function routeSubscriptionById(req, res, id) {
  ...
  let query = supabaseAdmin.from('subscriptions').select('*, plans(*)').eq('id', id)
...
async function routeSubscriptionStatus(req, res, id, status) {
  ...
  let query = supabaseAdmin.from('subscriptions').update({ status }).eq('id', id)

  if (role !== 'admin') query = query.eq('user_id', userId)

  const { data, error } = await query.select('*, plans(*)').single()
...
async function routeContacts(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const { data, error } = await supabaseAdmin
      .from('contacts')
      .insert({ user_id: userId, name, email, phone, tags: tags ?? [], list_id: list_id ?? null })
      .select()
      .single()
    if (error) return fail(res, error.message)
    return ok(res, { ...data, _id: data.id }, 201)
...
async function routeContactById(req, res, id) {
  ...
  if (req.method === 'PUT') {
    ...
    const { data, error } = await supabaseAdmin
      .from('contacts')
      .update({ name, email, phone, tags, list_id })
      .eq('id', id)
      .eq('user_id', userId)
      .select()
      .single()
...
async function routeContactListRsvp(req, res, listId) {
  ...
  const { data, error } = await supabaseAdmin
    ...
    .upsert(
      { list_id: listId, contact_id, status: status ?? 'pending' },
...
async function routeEntities(req, res) {
  ...
  if (req.method === 'GET') {
    const { data, error } = await supabaseAdmin
      .from('entities')
      .select('*, entity_members(*)')
      .or(`owner_id.eq.${userId},entity_members.user_id.eq.${userId}`)
      .order('created_at', { ascending: false })
    if (error) return fail(res, error.message)
    return ok(
      res,
      (data ?? []).map((e) => ({ ...e, _id: e.id }))
    )
  }
  ...
  if (req.method === 'POST') {
    ...
    const { data, error } = await supabaseAdmin
      .from('entities')
      .insert({ owner_id: userId, name, type, description, logo })
      .select()
      .single()
    if (error) return fail(res, error.message)
    ...
    await supabaseAdmin
      .from('entity_members')
      .insert({ entity_id: data.id, user_id: userId, role: 'admin' })
    return ok(res, { ...data, _id: data.id }, 201)
  }
...
async function routeEntityById(req, res, id) {
  ...
  if (req.method === 'GET') {
    const { data, error } = await supabaseAdmin
      .from('entities')
      .select('*, entity_members(*, users(name, email, image))')
      .eq('id', id)
      .single()
  ...
  if (req.method === 'PUT') {
    ...
    const { data, error } = await supabaseAdmin
      .from('entities')
      .update({ name, type, description, logo })
      .eq('id', id)
...
async function routeEntityMembers(req, res, entityId, memberId) {
  ...
  if (req.method === 'POST') {
    ...
    const { data, error } = await supabaseAdmin
      .from('entity_members')
      .insert({ entity_id: entityId, user_id, role: role ?? 'member' })
      .select()
      .single()
    if (error) return fail(res, error.message)
    return ok(res, { ...data, _id: data.id }, 201)
...
async function routeIntegrations(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const { data, error } = await supabaseAdmin
      .from('integrations')
      .insert({ user_id: userId, name, type, config: config ?? {}, enabled: enabled ?? true })
      .select()
      .single()
    if (error) return fail(res, error.message)
    return ok(res, { ...data, _id: data.id }, 201)
...
async function routeProAnalytics(req, res) {
  ...
  const [artworksRes, txRes, clientsRes, deliveryRes] = await Promise.all([
    supabaseAdmin
      .from('artworks')
      .select('id, status, for_sale, sold, price, currency')
  ...
  return ok(res, {
    artworks: {
      total: artworks.length,
      forSale: artworks.filter((a) => a.for_sale && !a.sold).length,
      sold: artworks.filter((a) => a.sold).length,
    },
    revenue: {
      total: totalRevenue,
      currency: completedTxs[0]?.currency ?? 'XOF',
      transactions: txs.length,
      completed: completedTxs.length,
    },
    clients: { total: clientsRes.data?.length ?? 0 },
    deliveries: {
      total: deliveryRes.data?.length ?? 0,
      pending: (deliveryRes.data ?? []).filter((d) => d.status === 'pending').length,
    },
  })
}
...
async function routeCampaignsCrud(req, res, id, action) {
  ...
  if (id && action === 'send-test' && req.method === 'POST') {
    ...
    try {
      await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: `Kucibok <${process.env.ADMIN_EMAIL}>`,
          to: [test_email],
      ...
      return fail(res, 'Erreur envoi test: ' + e.message)
    }
  ...
  if (id) {
    ...
    if (req.method === 'PUT') {
      ...
      const { data, error } = await supabaseAdmin
        .from('campaigns')
        .update({ name, subject, content, html_content, list_id })
        .eq('id', id)
        .eq('user_id', userId)
        .select()
        .single()
  ...
  if (req.method === 'POST') {
    ...
    const { data, error } = await supabaseAdmin
      .from('campaigns')
      .insert({ user_id: userId, name, subject, content, html_content, list_id, status: 'draft' })
      .select()
      .single()
    if (error) return fail(res, error.message)
    return ok(res, { ...data, _id: data.id }, 201)
