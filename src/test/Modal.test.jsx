...
describe('Modal', () => {
  ...
  it('affiche le contenu quand isOpen=true', () => {
    render(<Modal isOpen onClose={vi.fn()} title="Test"><p>Contenu modal</p></Modal>)
    expect(screen.getByText('Contenu modal')).toBeInTheDocument()
  })

  it('affiche le titre', () => {
    render(<Modal isOpen onClose={vi.fn()} title="Mon titre"><p>-</p></Modal>)
    expect(screen.getByText('Mon titre')).toBeInTheDocument()
  })

  it('appelle onClose au clic sur le bouton de fermeture', () => {
    const onClose = vi.fn()
    render(<Modal isOpen onClose={onClose} title="Test"><p>-</p></Modal>)
  ...
  it('appelle onClose au clic sur le backdrop', () => {
    const onClose = vi.fn()
    const { container } = render(<Modal isOpen onClose={onClose} title="Test"><p>-</p></Modal>)
  ...
  it("ne ferme pas au clic backdrop quand closeOnBackdrop=false", () => {
    ...
    const { container } = render(
      <Modal isOpen onClose={onClose} title="Test" closeOnBackdrop={false}><p>-</p></Modal>
    )
    const backdrop = container.querySelector('[aria-hidden="true"]')
    fireEvent.click(backdrop)
  ...
  it("n'appelle pas onClose à Escape quand closeOnEsc=false", () => {
    const onClose = vi.fn()
    render(<Modal isOpen onClose={onClose} title="Test" closeOnEsc={false}><p>-</p></Modal>)
    fireEvent.keyDown(document, { key: 'Escape' })
    expect(onClose).not.toHaveBeenCalled()
  })
  ...
  it('désactive le scroll du body quand ouvert', () => {
    render(<Modal isOpen onClose={vi.fn()} title="Test"><p>-</p></Modal>)
    expect(document.body.style.overflow).toBe('hidden')
  })

  it('restaure le scroll du body à la fermeture', () => {
    const { rerender } = render(<Modal isOpen onClose={vi.fn()} title="Test"><p>-</p></Modal>)
    rerender(<Modal isOpen={false} onClose={vi.fn()} title="Test"><p>-</p></Modal>)
...
describe('ConfirmDialog', () => {
  ...
  it('appelle onClose au clic sur Annuler', () => {
    const onClose = vi.fn()
    render(
      <ConfirmDialog isOpen onClose={onClose} onConfirm={vi.fn()} message="Supprimer ?" />
    )
