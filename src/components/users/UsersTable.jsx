        ...
        <table className="w-full text-sm text-kcb-sable min-w-[600px]">
          <thead>
            <tr className="border-b border-white/[0.06]">
              <th className="font-medium py-2 px-2 text-left whitespace-nowrap text-white/70">
                Nom
              </th>
              <th className="font-medium py-2 px-2 text-left whitespace-nowrap text-white/70">
                Email
              ...
              <th className="font-medium py-2 px-2 text-right whitespace-nowrap text-white/70">
                Actions
              </th>
            </tr>
          </thead>
          {users?.length >= 1 ? (
            ...
            <tfoot>
              <tr>
                <td colSpan={7} className="text-center text-kcb-pierre text-sm py-8">
      ...
      {state.users.length > 5 && (
        <div className="flex justify-end gap-2 mt-4">
          ...
          <span className="text-xs text-kcb-pierre flex items-center px-2">
            Page {Math.floor(state.users.indexOf(state.set[0]) / 40) + 1} /{' '}
            {Math.ceil(state.users.length / 40)}
          </span>
          <button
            className="rounded-[4px] px-3 py-1 text-xs text-white/60 bg-kcb-ardoise hover:bg-kcb-ardoise transition disabled:opacity-30 shadow-none border-none"
            onClick={() => {
