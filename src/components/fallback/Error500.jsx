...
export default function Error500({error}) {
    ...
    useEffect(() => {
        const sendReport = async () => {
            try {
                await sendErrorReport({ error: "Erreur 500", errorInfo: error?.toString() || "Aucune information disponible" });
            } catch (err) {
                console.error("Erreur lors de l'envoi du rapport d'erreur :", err);
            }
        };
        sendReport();
    }, []);
      ...
      <div className="max-w-md w-full bg-card/80 backdrop-blur-sm rounded-xl overflow-hidden shadow-2xl border border-gray-700 animate-fade-in">
        <div className="p-6 sm:p-10">
          <div className="flex flex-col items-center text-center">
            <h1 className="font-serif text-8xl font-bold text-gradient mb-2">500</h1>
            <h2 className="text-2xl font-bold mb-4">Erreur interne du serveur</h2>
            <div className="w-16 h-1 bg-gradient-to-r from-earth to-clay mb-6"></div>
            
            <p className="text-muted-foreground mb-8">
            Oups ! On dirait que quelque chose a mal tourné en coulisses. Veuillez réessayer plus tard ou contacter l'administrateur si le problème persiste. La galerie <span className="font-serif font-bold text-primary">Kucibok</span> est toujours là pour vous.
            </p>
