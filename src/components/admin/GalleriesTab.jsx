  ...
  return (
    <div className="rounded-[4px] bg-kcb-ardoise shadow-none p-6">
      ...
      <div className="mb-2 flex gap-4 text-sm text-kcb-pierre">
        ...
        <span>
          Avec email : <span className="font-bold text-green-400">{filtered}</span>
        </span>
        <span>
          Affichées : <span className="font-bold text-kcb-or">{state.galleries.length}</span>
        </span>
      </div>
              ...
              <tbody>
                {state.set.map((g) => (
                  <tr key={g?._id} className="hover:bg-kcb-ardoise/40">
                    <td className="border px-2 py-2 font-bold text-white/90">{g?.name}</td>
                    <td className="border px-2 py-2 text-xs">{g?.email || '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {state.galleries.length > 40 && (
            <div className="flex justify-end gap-2 mt-4">
              ...
              <span className="text-xs text-kcb-pierre flex items-center px-2">
                Page {Math.floor(state.galleries.indexOf(state.set[0]) / 40) + 1} /{' '}
              ...
              <button
                ...
                disabled={
                  state.set[state.set.length - 1] === state.galleries[state.galleries.length - 1]
                }
              >
