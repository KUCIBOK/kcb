...
export function Analytics({ currency = 'EUR' }) {
  ...
  const defaultData = {
    ...
    totalUsers: 7154, mau: 5500, dau: 3668, acquisition_growth: 15,
    channels: {
      organic:  { pct: 45, roi: 3.2 },
      paid:     { pct: 35, roi: 1.8 },
      referral: { pct: 20, roi: 4.1 },
    },
    totalArtworks: 236, artworks_growth: 18,
    artworks_by_category: { peintures: 120, sculptures: 65, digital: 51 },
    artworks_with_cert: 215, quality_score: 87,
    gmv: 1707, aov: 279, conversion_rate: 3.8,
    views_total: 12450, favorites: 856, messages: 234, contacts: 89, sales: 6,
    commission_revenue: 341,
    logistics: {
      label: 'Q4 2025', envois: 110, envois_mensuels: 9,
      panier_moyen: 279, revenu_mensuel: 2561, marge: 71.4,
    },
    dau_mau_ratio: 66.7, retention_30d: 72, retention_7d: 89,
    feature_adoption: { enchères: 78, favoris: 92, messagerie: 65 },
    at_risk_users: 7,
    uptime: 99.8, api_response_time: 145, error_rate: 0.2,
    lcp: 1.8, fid: 35, cls: 0.08, certs_generated: 215,
    nps: 72, csat: 4.3, total_tickets: 45,
    avg_response_time: 2.5, avg_resolution_time: 24,
    ticket_categories: { facturation: 12, technique: 18, général: 15 },
    churn_risk_users: 7, upsell_opportunities: 7,
    pendingArtworks: null, openTickets: null,
    alerts: [
  ...
  const loadLiveData = useCallback(async (p) => {
    ...
    } catch (_) { /* fallback sur defaultData */ }
  }, [])
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      {liveData ? (
        <div className="flex items-center gap-2 text-xs text-green-400 bg-green-500/10 border border-green-500/20 rounded-[4px] px-3 py-2 w-fit">
          ...
          <span className="text-green-300/60 ml-1">
            · {liveData.transactions?.completed ?? 0} transactions · {liveData.artworks?.total ?? 0} œuvres
          </span>
        </div>
      ) : (
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <HeroKPI icon={TrendingUp} label="MRR" value={fmt(merged.mrr)} change={`+${merged.mrrGrowth}% MoM`} trend="up" />
        <HeroKPI icon={Users} label="Utilisateurs" value={merged.totalUsers.toLocaleString('fr-FR')} change={`+${merged.acquisition_growth}% ce mois`} trend="up" />
        <HeroKPI icon={Package} label="Œuvres" value={merged.totalArtworks} change={`${merged.artworks_with_cert} certifiées`} trend="up" />
        <HeroKPI icon={BarChart2} label="GMV" value={fmt(merged.gmv, { compact: true })} change="Volume mensuel" trend="up" />
      </div>
      ...
      <Section title="Revenue & Finance">
        ...
        <div className="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <MetricCard label="Payback Period" value={`${merged.payback_period} mois`} change="Temps de récupération du CAC" color="indigo" />
          <MetricCard label="MRR M+1" value={fmt(merged.revenue_projection_3m)} change="MRR × 1,12 (+12% MoM)" color="emerald" />
        </div>
      </Section>
      ...
      <Section title="SaaS — Abonnements">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard
            ...
            color="green"
          />
          {merged.saas_segments && Object.entries(merged.saas_segments).map(([seg, info]) => (
            <div key={seg} className="bg-kcb-ardoise/50 border border-white/[0.06] rounded-[4px] p-4">
              ...
              <p className="text-2xl font-bold text-white">
                {info.count} <span className="text-base font-normal text-kcb-pierre">abonnés</span>
              </p>
          ...
          <div className="overflow-x-auto bg-kcb-ardoise/50 border border-white/[0.06] rounded-[4px] p-4">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-white/[0.03]">
                ...
                {[
                  ['SaaS', 'saas'], ['Marketplace', 'marketplace'],
                  ['Logistique', 'logistique'], ['Numérisation', 'numerisation'],
                ...
                {[
                  ['Salaires', 'salaires'], ['Tech', 'tech'], ['Marketing', 'marketing'],
                  ['Logistique', 'logistique'], ['Numérisation', 'numerisation'],
                  ['Admin', 'admin'], ['Amortissement', 'amort'], ['Divers', 'divers'],
                ].map(([label, key]) => (
                  <tr key={key} className="hover:bg-white/[0.02]">
                    <td className="py-1.5 pr-4 text-kcb-sable">{label}</td>
                    {Q1_2026.pl_monthly.map(m => (
                ...
                <tr className="border-t-2 border-kcb-or/40">
                  ...
                  {Q1_2026.pl_monthly.map(m => (
                    <td key={m.mois} className={`py-3 px-3 text-right font-bold ${m.net >= 0 ? 'text-green-300' : 'text-red-300'}`}>
                      {fmt(m.net)}
                    </td>
                  ))}
      ...
      <Section title="Utilisateurs & Croissance">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard label="Total utilisateurs" value={merged.totalUsers.toLocaleString('fr-FR')} change={`+${merged.acquisition_growth}% ce mois`} trend="up" color="blue" />
          <MetricCard label="MAU" value={merged.mau.toLocaleString('fr-FR')} change="Actifs / mois" color="indigo" />
          <MetricCard label="DAU" value={merged.dau.toLocaleString('fr-FR')} change="Actifs / jour" color="violet" />
              ...
              {Object.entries(merged.channels).map(([channel, metrics]) => (
                <div key={channel}>
                  <div className="flex justify-between text-xs mb-1">
                    ...
                    <span className="text-white font-semibold">{metrics.pct}% · ROI {metrics.roi}x</span>
                  </div>
      ...
      <Section title="Contenu & Inventaire">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard label="Œuvres publiées" value={merged.totalArtworks} change={`+${merged.artworks_growth}% ce mois`} trend="up" color="purple" />
          <div className="bg-kcb-ardoise/50 border border-white/[0.06] rounded-[4px] p-4">
          ...
          <MetricCard
            ...
            color="green"
          />
          <MetricCard label="Score qualité moyen" value={`${merged.quality_score}%`} change="Complétude des fiches" color="amber" />
        </div>
      </Section>
      ...
      <Section title="Marketplace & Ventes">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard label="GMV" value={fmt(merged.gmv, { compact: true })} change="Volume marchand mensuel" color="green" />
          <MetricCard label="AOV" value={fmt(merged.aov)} change="Valeur moy. / commande" color="blue" />
          <MetricCard label="Commission (20%)" value={fmt(merged.commission_revenue)} change="Revenu net commission" color="amber" />
          <MetricCard label="Taux de conversion" value={`${merged.conversion_rate}%`} change="Vues → Vente" color="purple" />
        </div>
        <div className="mt-4">
          <FunnelChart
      ...
      {merged.logistics && (
        <Section title={`Logistique — ${merged.logistics.label}`}>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <MetricCard label="Expéditions" value={merged.logistics.envois} change={`${merged.logistics.envois_mensuels} envois/mois`} color="blue" />
            <MetricCard label="Panier moyen" value={fmt(merged.logistics.panier_moyen)} change="Par envoi" color="indigo" />
            <MetricCard label="Revenu logistique" value={fmt(merged.logistics.revenu_mensuel)} trend="up" color="green" />
            <MetricCard label="Marge nette" value={`${merged.logistics.marge}%`} change="Marge sur expéditions" trend="up" color="emerald" />
          </div>
        </Section>
      )}
      ...
      <Section title="Engagement & Rétention">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard label="Stickiness DAU/MAU" value={`${merged.dau_mau_ratio.toFixed(1)}%`} change="Ratio quotidien/mensuel" trend={merged.dau_mau_ratio > 50 ? 'up' : 'down'} color="indigo" />
          <MetricCard label="Rétention 7J" value={`${merged.retention_7d}%`} change="Retour à 7 jours" color="green" />
          <MetricCard label="Rétention 30J" value={`${merged.retention_30d}%`} change="Retour à 30 jours" color="blue" />
          <MetricCard label="Utilisateurs à risque" value={merged.at_risk_users} change="Churn probable" trend="down" color="red" />
        </div>
        <div className="mt-4 bg-kcb-ardoise/50 border border-white/[0.06] rounded-[4px] p-4">
          ...
          <div className="space-y-2">
            {Object.entries(merged.feature_adoption).map(([feature, adoption]) => (
              <ProgressBar key={feature} label={feature.charAt(0).toUpperCase() + feature.slice(1)} value={adoption} />
            ))}
          </div>
        </div>
      </Section>
      ...
      <Section title="Technique & Performance">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard label="Uptime" value={`${merged.uptime}%`} change="Disponibilité du service" trend="up" color="green" />
          <MetricCard label="API Response" value={`${merged.api_response_time}ms`} change="Temps de réponse moyen" color="blue" />
          <MetricCard label="Error Rate" value={`${merged.error_rate}%`} change="Taux d'erreur" trend="down" color="red" />
          <MetricCard label="Certificats PDF" value={merged.certs_generated} change="Générés au total" color="purple" />
        </div>
        <div className="mt-4 bg-kcb-ardoise/50 border border-white/[0.06] rounded-[4px] p-4">
      ...
      <Section title="Support & Satisfaction">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard label="NPS" value={merged.nps} change="Score promoteur net" trend="up" color="green" />
          <MetricCard label="CSAT" value={`${merged.csat.toFixed(1)}/5`} change="Satisfaction client" color="blue" />
          <MetricCard label="Tickets ouverts" value={merged.total_tickets} change="Demandes de support" color="purple" />
          <MetricCard label="Temps résolution" value={`${merged.avg_resolution_time}h`} change="Délai moyen" color="indigo" />
        </div>
        <div className="mt-4 bg-kcb-ardoise/50 border border-white/[0.06] rounded-[4px] p-4">
      ...
      <Section title="Prédictions">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <MetricCard label="MRR M+1" value={fmt(merged.revenue_projection_3m)} change="MRR × 1,12 (+12% MoM)" color="green" />
          <MetricCard label="Churn probable" value={merged.churn_risk_users} change="Utilisateurs à risque de départ" trend="down" color="orange" />
          <MetricCard label="Upsell identifiés" value={merged.upsell_opportunities} change="Comptes éligibles plan supérieur" color="purple" />
        </div>
      </Section>
...
function Section({ title, children }) {
  return (
    <div className="space-y-4">
      <h2 className="text-xl font-bold text-white font-playfair border-b border-white/[0.06] pb-2">{title}</h2>
      {children}
    </div>
  )
}
...
function MetricCard({ label, value, change, trend, color = 'gray' }) {
  ...
  return (
    <div className={`border rounded-[4px] p-4 ${cls[color] ?? 'bg-white/[0.03] border-white/[0.06]'}`}>
        ...
        {Object.entries(data).map(([key, value], idx) => (
          <div key={key}>
            ...
            <div className="h-2 bg-kcb-ardoise rounded-full overflow-hidden">
              <div className="h-full" style={{ width: `${(value / total) * 100}%`, backgroundColor: colors[idx % colors.length] }} />
            </div>
          </div>
        ))}
      </div>
...
function FunnelChart({ title, stages }) {
  ...
  const colorMap = { blue: '#C9A84C', purple: '#8B6914', indigo: '#C9A84C', amber: '#d97706', green: '#10b981' }
  return (
    <div className="bg-kcb-ardoise/50 border border-white/[0.06] rounded-[4px] p-4">
        ...
        {stages.map((stage, idx) => (
          <div key={idx}>
            <div className="flex justify-between text-sm mb-1">
              ...
              <span className="text-white font-semibold">{stage.value.toLocaleString('fr-FR')}</span>
            </div>
            <div className="h-3 bg-kcb-ardoise rounded-full overflow-hidden">
              <div
                className="h-full rounded-full"
                style={{ width: `${(stage.value / maxValue) * 100}%`, backgroundColor: colorMap[stage.color] }}
              />
            </div>
...
function VitalCard({ label, value, target, status }) {
  return (
    <div className="bg-kcb-ardoise/50 rounded-[4px] p-3 text-center border border-white/[0.06]">
      ...
      <p className={`text-2xl font-bold ${status === 'good' ? 'text-green-400' : 'text-yellow-400'}`}>{value}</p>
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-2">
        ...
        {badge != null && (
          <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${badgeCls[badgeColor]}`}>{badge}</span>
        )}
      </div>
