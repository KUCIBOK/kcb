...
export function CampainTab() {
  const { artists, buyers, curators } = useUsersContext()
  const totalAudience = (artists?.length ?? 0) + (buyers?.length ?? 0) + (curators?.length ?? 0)
    ...
    <div className="flex flex-col lg:flex-row gap-4 items-start">
      <section className="border rounded-[4px] shadow-lg p-6 w-full md:w-6/10">
        <header className="flex items-center gap-2 mb-6">
          <span className="inline-flex items-center justify-center bg-kcb-or/10 rounded-full p-2">
            <Mail className="w-5 h-5 text-kcb-or" />
          </span>
      ...
      <section className="border rounded-[4px] shadow-lg p-4 w-full md:w-4/10">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="text-sm font-bold px-2 bg-kcb-or text-kcb-noir rounded-full">
            {' '}
            {totalAudience}{' '}
          </div>
        </div>
