...
export function EnhancedDeliveryRequestList({ deliveries }) {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'on_the_way': return 'text-kcb-or';
      case 'in_preparation': return 'text-yellow-400';
      case 'pending': return 'text-kcb-pierre';
  ...
  const getLogidooStatusBadge = (logidooStatus) => {
    ...
    return (
      <span className={`px-2 py-1 rounded-full text-xs ${statusColors[logidooStatus] || 'bg-kcb-bronze/20 text-kcb-bronze'}`}>
          ...
          <tbody>
            {state?.set?.length > 0 ? (
              state?.set?.map((delivery) => {
                ...
                return (
                  <tr key={delivery._id} className="border-b border-white/[0.06] hover:bg-kcb-ardoise/50 transition">
                    <td className="p-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          {delivery.trackingId && (
                            <div className="text-xs text-kcb-pierre">KCB-{delivery._id.slice(-6)}</div>
                          )}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="p-3">
                      <div className={`flex items-center gap-2 ${getStatusColor(delivery.status)}`}>
                        <div className={`w-2 h-2 rounded-full ${
                          ...
                          delivery.status === 'on_the_way' ? 'bg-kcb-or' :
                          ...
                          {logidooData ? (
                            <div className="space-y-1">
                              ...
                              {logidooData.provider && (
                                <div className="text-xs text-kcb-bronze">Via {logidooData.provider}</div>
                              )}
                            </div>
                          ) : delivery.trackingId ? (
              ...
              <tr>
                <td colSpan={state.showLogidooData ? "9" : "7"} className="text-center py-8 text-kcb-pierre">
