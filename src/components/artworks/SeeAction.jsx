...
export const SeeAction = memo(({artwork, user}) => {
    ...
    return(
        <>
        <button title="Voir" onClick={()=> setShow(!show)} className="border border-white/[0.06] rounded-md bg-kcb-ardoise hover:bg-kcb-ardoise p-2 flex items-center gap-3">
        ...
        <div className="relative bg-kcb-ardoise border border-zinc-800 overflow-auto h-[90vh] rounded-[4px] shadow-xl w-full max-w-2xl p-0 animate-scale-up">
            ...
            <div className="px-8 pt-8 pb-2">
                <div className="mb-6">
                    ...
                    <p className="text-xs text-kcb-pierre mb-2">par <span className="font-medium text-white">{artwork.artist}</span></p>
                </div>
                    ...
                    <div className="lg:w-1/2 flex flex-col gap-2">
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                            {artwork?.tags?.map((tag, index) => (
                                <span key={index} className="px-3 py-1 rounded-full font-semibold text-xs bg-kcb-or text-white">{tag}</span>
                            ))}
                        </div>
