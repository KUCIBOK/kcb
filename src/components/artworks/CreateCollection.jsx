...
function AddCollectionModal({closeModal}){
    ...
    const handleSubmit = async function (e){
        ...
        try {
            if(state.description.length < 20) setState({...state, error : 'Remplissez la description (min 20)'})
                ...
                <div className="lg:flex py-4 gap-5">
                    <form onSubmit={handleSubmit} method="post" className="py-4 w-full">
                        ...
                        <div className="flex flex-col gap-1">
                            <label htmlFor="description" className="text-sm text-white font-semibold">Titre de la collection</label>
                            <input onChange={(e) => setState({...state, title : e.target.value})} value={state.title}  type="text" placeholder="Série Horizons urbains 2023" className="py-1.5 px-3 rounded-md shadow-lg border text-white" required minLength={10} />
                        </div>
                        <div className="flex flex-col mt-4">
                            <label htmlFor="description" className="text-sm text-white font-semibold">Description</label>
                            <ReactQuill
                                theme="snow"
                            ...
                            <div className="grid mt-4">
                                ...
                                <select onChange={(e) => setState({...state, artist : e.target.value, artistId : myArtists.find(item => item?.name == e.target.value)?._id, artworks : state.artworks.map(item => ({...item, artist : e.target.value, artistId : myArtists.find(item => item?.name == e.target.value)?._id}))})} name="artist" id="artist" className="rounded-md bg-kcb-ardoise mt-1 border border-white/[0.06] px-3 py-1.5" required>
                                    ...
                                    {myArtists?.length > 0 ? myArtists?.map((artist, index) => (
                                        ...
                                        <option disabled>Vous n'avez aucun artiste, ajoutez un artiste pour commencer</option>
                                    }
                                </select>
                            </div>
                        )}
                        ...
                        <div className="flex justify-end items-center gap-4 mt-8">
                            ...
                            <button type="submit" className="rounded-md bg-green-600 hover:opacity-90 flex items-center gap-3 py-2 px-3">
                                {state?.loading ? <DataLoader/> : (
                                    <>
...
function ArtworkShortList({state, setState}){
    ...
    return (
        <>
        {state?.artworks?.length > 0 ? state?.artworks?.map((artwork, index) => (
           <div key={index}>
...
function ArtworkShortListItem({setFormState, formState, artwork, index}){
    ...
    useEffect(() => {
        setFormState(prev => ({...prev, artworks : formState.artworks.map((item, idx) => {
            if(idx == index){
                return state
            }
            return item
    ...
    return (
        ...
        <form className="rounded-[4px] bg-kcb-ardoise my-4 shadow-lg">
            <div>
                ...
                :
                <div onDragOver={handleDragOver} onDrop={handleDrop} onClick={() => document.getElementById('artwork-image').click()} className="flex justify-center items-center bg-gray-400/70 rounded-t-lg h-80">
            ...
            <div className="p-4">
                ...
                <div className="flex mt-4">
                    <label htmlFor="forSale">
                        <input onChange={(e) => setState({...state, forSale : e.target.checked})} checked={state.forSale} type="checkbox" id="forSale" className="mr-2" />
                        En vente ?
                    </label>
                </div>
                ...
                <div className="flex flex-col mt-4">
                    ...
                    <div className="flex flex-wrap">
                        {state.tags.map((tag, index) => (
                            <span key={index} className="bg-purple-700/50 flex animate-slide-left items-center gap-2 text-white rounded-full px-3 py-1 text-sm font-semibold mt-2 mr-2">{tag} <span onClick={() => setState({...state, tags : state.tags.filter(item => !(item == tag))})} className="text-xs cursor-pointer">x</span> </span>
                        ))}
                    </div>
                    <div  className="flex my-4">
                        <button onClick={() => setFormState({...formState, artworks : formState.artworks.filter((item, idx) => idx != index)})} type="button" className="p-2 bg-red-900 rounded-md">
