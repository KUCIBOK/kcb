...
export default function PortalHero({ label, title, subtitle, actions = [], children }) {
  return (
    <section className="relative overflow-visible pt-36 pb-0">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-start">
          <div>
            ...
            <RevealOnScroll delay={0.1}>
              <h1 className="font-playfair font-black text-[clamp(44px,5.5vw,72px)] leading-none tracking-tight mb-7 mt-4">
                {title}
              </h1>
            </RevealOnScroll>
            <RevealOnScroll delay={0.2}>
              <p className="text-base font-light leading-[1.8] text-kcb-pierre max-w-[460px] mb-12">
                {subtitle}
              </p>
            </RevealOnScroll>
            <RevealOnScroll delay={0.3}>
              <div className="flex gap-4 flex-wrap pb-24">
                {actions.map((a, i) => {
                  const cls = a.primary
                    ? "inline-flex items-center gap-2 bg-[var(--accent)] text-kcb-noir font-dm-sans font-semibold text-xs tracking-[0.08em] uppercase px-9 py-3.5 transition-all hover:bg-[var(--accent-dark)] hover:-translate-y-px"
