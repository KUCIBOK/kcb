    ...
    <div className="group relative overflow-hidden rounded-[4px] bg-kcb-ardoise border border-white/[0.06] shadow-md hover:shadow-lg transition">
      <Link to={`/blog/${post?._id}`} className="block">
        ...
        <div className="flex flex-col gap-1 p-4">
          ...
          <h4 className="font-serif font-semibold text-base text-white mb-1 truncate">
            {post?.title}
          </h4>
          ...
          {post?.tags?.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">
              {post.tags.map((tag, index) => (
                <span
                  key={index}
                  className="rounded-full text-xs border border-white/[0.06] px-2 py-0.5 text-kcb-pierre bg-kcb-noir/60"
                >
                  {tag}
                </span>
              ))}
