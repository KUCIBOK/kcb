      ...
      <div className="space-y-3 mb-6">
        {ROLES.map(({ key, icon: Icon, label, description, badge }) => {
          ...
          return (
            ...
            >
              {/* Icon */}
              <span className={`mt-0.5 p-2 transition-colors ${selected ? "bg-kcb-or/20 text-kcb-or" : "bg-white/[0.04] text-kcb-pierre"}`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  <span className={`text-sm font-semibold ${selected ? "text-white" : "text-kcb-sable"}`}>{label}</span>
              ...
              {/* Checkmark */}
              <span className={`mt-0.5 w-5 h-5 rounded-full border flex items-center justify-center shrink-0 transition-all duration-200 ${
