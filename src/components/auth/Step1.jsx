...
export const Step1 = ({ formState, setFormState, onGoogleSignup }) => (
  <div className="bg-kcb-ardoise border border-white/[0.06] p-8 relative overflow-hidden">
    ...
    <button
      ...
      onClick={() =>
        setFormState((p) => ({ ...p, connectMethod: 'email', error: '', step: 1, loading: false }))
      }
      disabled={formState.loading}
      type="button"
    >
    ...
    <div className="mt-8 pt-6 border-t border-white/[0.04] flex items-center justify-between">
      <Link to={-1} className="text-xs text-kcb-pierre hover:text-white transition">
        ← Retour
      </Link>
      <Link to="/sign-in" className="text-xs text-kcb-or hover:underline">
        Déjà inscrit ?
      </Link>
    </div>
  </div>
