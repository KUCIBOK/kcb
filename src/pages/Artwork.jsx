...
export default function Artwork() {
    ...
    const sameArtist = useMemo(() => {
        ...
        return forSale
            .filter(
                (a) =>
                    (a.artist_id === artistId || a.artistId === artistId) &&
                    (a._id || a.id) !== id
            )
            .slice(0, RECO_LIMIT);
    }, [artwork?.artist_id, artwork?.artistId, forSale, id]);
    ...
    const similar = useMemo(() => {
        ...
        return forSale
            ...
            .slice(0, RECO_LIMIT);
    }, [artwork?.category, artwork?.artist_id, artwork?.artistId, forSale, id]);
    ...
    return (
        <>
            <Helmet>
                ...
                {artwork?.kucibok_id && <meta property="product:retailer_item_id" content={artwork.kucibok_id} />}
            </Helmet>
            <main className="max-w-6xl mx-auto px-4 md:px-6 py-8">
                <div className="mb-6">
                    <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-kcb-pierre hover:text-white text-sm font-medium transition">
                            ...
                            <div className="md:w-7/12 w-full flex flex-col items-center md:items-start">
                                <div className="relative w-full max-w-full md:max-w-lg aspect-square rounded-[4px] overflow-hidden border border-white/[0.06] bg-kcb-noir shadow-lg">
                                    <img
                                        ...
                                        onError={(e) => { e.target.src = '/images/placeholder-artwork.svg'; }}
                                    />
                                    <button
                                ...
                                <div>
                                    ...
                                    <div className="border border-kcb-or/20 rounded-[4px] p-4 md:p-6 bg-kcb-ardoise flex flex-col gap-3 shadow-xl mt-2">
                                        <div className="flex flex-wrap items-center justify-between gap-3 mb-2">
                                            <span className="font-playfair text-xl md:text-2xl text-white font-semibold">{new Intl.NumberFormat('fr-FR', { style: 'currency', currency: artwork?.currency || 'XOF' }).format(artwork?.price || 0)}</span>
                                            <Link
                                    ...
                                    <div className="mt-6">
                                        ...
                                        <ul className="text-sm text-kcb-sable space-y-1">
                                            ...
                                            <li className="flex justify-between"><span>Créé le</span><span className="text-white">{artwork?.created ? new Date(artwork.created).toLocaleDateString('fr-FR') : '—'}</span></li>
                                    ...
                                    <div className="mt-4">
                                        ...
                                        <p id="artwork-description" className="text-kcb-sable text-sm leading-relaxed" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(artwork?.description ?? '') }}></p>
                                    </div>
                ...
                {artwork?.error && (
                    <div className="text-center py-12 px-6 border border-dashed border-white/[0.06] rounded-[4px] bg-kcb-ardoise/30 mt-10">
                        ...
                        <div className="my-6">
                            <Link to={`${portal}/catalogue`} className="bg-kcb-or hover:bg-kcb-bronze rounded-[4px] py-2 px-4 text-kcb-noir font-semibold transition uppercase tracking-[0.05em] text-sm">Aller sur le catalogue</Link>
                        </div>
                    </div>
                )}
            </main>
