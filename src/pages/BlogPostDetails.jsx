    ...
    <>
      ...
      <div className="w-full lg:w-7/10 px-4 lg:px-6 my-6 mx-auto">
        <div className="flex justify-start items-center mx-auto">
          <Link
            to={-1}
            className="rounded-[4px] p-2 text-sm gap-1 text-white hover:bg-white/[0.03] flex items-center"
          >
        ...
        {state?.post?._id && (
          <RevealOnScroll>
            <div className="flex flex-col w-full mx-auto py-4">
              <h2 className="font-playfair text-[clamp(24px,3vw,36px)] font-bold text-white mb-2 tracking-tight">
                {state?.post?.title}
              </h2>
              ...
              {state?.post?.image && (
                <img
                  ...
                  className="w-full object-cover rounded-[4px] h-56 sm:h-72 md:h-80 lg:h-96 mb-6 border border-white/[0.06]"
                  onError={(e) => {
                ...
                <div className="border-b border-white/[0.06] pb-6">
                  <span
                    ...
                    dangerouslySetInnerHTML={{
                      __html: DOMPurify.sanitize(state?.post?.content ?? '', {
                        ALLOWED_TAGS: [
                          'p',
                          'br',
                          'strong',
                ...
                {state?.post?.tags?.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {state?.post?.tags?.map((tag, index) => (
                      <span
                        key={index}
                        className="rounded-[4px] bg-kcb-or/10 text-xs text-kcb-or px-3 py-1 flex items-center gap-1 font-medium border border-kcb-or/20"
                      >
