      ...
      <div className="relative aspect-[3/4] overflow-hidden bg-kcb-ardoise">
        ...
        {artwork.kucibok_id && (
          <div className="absolute top-3 right-3 flex items-center gap-1 bg-[var(--accent)]/90 backdrop-blur-sm px-2 py-0.5">
            ...
            <span className="font-jetbrains text-[9px] font-bold text-kcb-noir tracking-[0.08em]">KCB</span>
          </div>
        )}
...
function CatalogueContent() {
  ...
  const clearFilters = useCallback(() => {
    setSearch(""); setCategory("All"); setForSaleOnly(false); setCertified(false)
    setSort("recent"); setVisible(PAGE_SIZE)
  }, [])

  const hasActive = search || category !== "All" || forSaleOnly || certifiedOnly || sort !== "recent"
        ...
        {!loading && (
          <RevealOnScroll delay={0.1}>
            <div className="flex flex-wrap gap-8 mt-8 text-sm">
              <div>
                <span className="text-[var(--accent)] font-bold text-2xl font-playfair">{allArtworks.length}</span>
                <span className="text-kcb-pierre ml-2 text-xs uppercase tracking-widest">{isFr ? "œuvres" : "works"}</span>
              </div>
              <div>
                <span className="text-[var(--accent)] font-bold text-2xl font-playfair">
                  {allArtworks.filter(a => a.kucibok_id).length}
                </span>
                <span className="text-kcb-pierre ml-2 text-xs uppercase tracking-widest">{isFr ? "certifiées KCB" : "certified KCB"}</span>
              </div>
              <div>
                <span className="text-[var(--accent)] font-bold text-2xl font-playfair">{allArtists.length}</span>
                <span className="text-kcb-pierre ml-2 text-xs uppercase tracking-widest">{isFr ? "artistes" : "artists"}</span>
              </div>
            </div>
          </RevealOnScroll>
        )}
        ...
        {!loading && (
          <div className="flex gap-0 mt-10 border-b border-white/[0.06]">
            {["artworks", "artists"].map(tab => (
              ...
              >
                {tab === "artworks"
                  ? (isFr ? `Œuvres (${allArtworks.length})` : `Artworks (${allArtworks.length})`)
                  : (isFr ? `Artistes (${allArtists.length})` : `Artists (${allArtists.length})`)}
              </button>
            ))}
          </div>
        )}
      ...
      <div className="max-w-[1280px] mx-auto px-[clamp(24px,5vw,80px)] py-10">
        ...
        <div className={`bg-kcb-ardoise border border-white/[0.05] p-4 mb-10 flex flex-wrap gap-3 items-center ${activeTab === "artists" ? "hidden" : ""}`}>
          <div className="relative flex-1 min-w-52">
            ...
            />
            {search && (
              <button onClick={() => { setSearch(""); setVisible(PAGE_SIZE) }} className="absolute right-3 top-1/2 -translate-y-1/2 text-kcb-pierre hover:text-white">
                <X className="w-4 h-4" />
              </button>
            )}
          </div>
          ...
          <div className="relative">
            ...
            >
              {categories.map((c) => <option key={c} value={c}>{c === "All" ? (isFr ? "Toutes" : "All") : c}</option>)}
            </select>
          ...
          <button
            onClick={() => { setForSaleOnly(!forSaleOnly); setVisible(PAGE_SIZE) }}
          ...
          <button
            onClick={() => { setCertified(!certifiedOnly); setVisible(PAGE_SIZE) }}
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32 gap-4">
            ...
            <p className="text-kcb-pierre text-sm">{isFr ? "Chargement de la collection…" : "Loading collection…"}</p>
          </div>
        ) : activeTab === "artists" ? (
          ...
          <div className="flex flex-col items-center justify-center py-32 gap-4 text-center">
            ...
            <button onClick={clearFilters} className="text-xs text-[var(--accent)] border border-[var(--accent)]/30 px-4 py-2 hover:bg-[var(--accent)]/5 transition">
