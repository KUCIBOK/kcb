...
const PURPOSES = {
  en: ["Exhibition / Fair", "Corporate Collection", "Gallery Programme", "Private Collection", "Museum Acquisition", "Other"],
  fr: ["Exposition / Foire", "Collection d'entreprise", "Programme de galerie", "Collection privée", "Acquisition muséale", "Autre"],
}

const BUDGETS = {
  en: ["< 5 000 EUR", "5 000 – 20 000 EUR", "20 000 – 100 000 EUR", "100 000 – 500 000 EUR", "> 500 000 EUR", "Undisclosed"],
  fr: ["< 5 000 EUR", "5 000 – 20 000 EUR", "20 000 – 100 000 EUR", "100 000 – 500 000 EUR", "> 500 000 EUR", "Non communiqué"],
}
...
function SourcingContent() {
  ...
  const [form, setForm] = useState({
    name: "", email: "", organization: "", purpose: "", budget: "", message: ""
  })
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.name || !form.email || !form.message) {
      setError(lang === "en" ? "Name, email and message are required." : "Nom, email et message sont requis.")
      return
    }
    setLoading(true)
  ...
  const inputCls = "w-full bg-kcb-noir border border-white/[0.08] px-4 py-3 text-sm text-white placeholder-kcb-pierre/40 focus:outline-none focus:border-[var(--accent)] transition"
    ...
    <div className="min-h-screen bg-kcb-noir-deep text-white pt-28">
      ...
      <div className="max-w-[1280px] mx-auto px-[clamp(24px,5vw,80px)] pb-20 border-b border-white/[0.04]">
        <RevealOnScroll>
          ...
          <p className="text-kcb-pierre text-[15px] leading-[1.8] max-w-xl">
            {t.desc}
          </p>
        </RevealOnScroll>
      </div>
        ...
        <RevealOnScroll>
          <div className="flex flex-col gap-8">
            ...
            <div className="mt-6 space-y-4 pt-6 border-t border-white/[0.04]">
              ...
              {[
                lang === "en"
                  ? { n: "01", t: "Submit your brief", d: "Tell us your programme, budget and timeline." }
                  : { n: "01", t: "Soumettez votre brief", d: "Dites-nous votre programme, budget et calendrier." },
                lang === "en"
                  ? { n: "02", t: "Curated selection", d: "Within 48h we send a selection of verified artists matching your criteria." }
                  : { n: "02", t: "Sélection sur mesure", d: "Sous 48h nous vous envoyons une sélection d'artistes vérifiés correspondant à vos critères." },
                lang === "en"
                  ? { n: "03", t: "End-to-end execution", d: "We handle contracts, logistics, customs, and delivery to your venue." }
                  : { n: "03", t: "Exécution complète", d: "Nous gérons les contrats, la logistique, les douanes et la livraison." },
              ].map((s) => (
                <div key={s.n} className="flex gap-4">
                  <span className="font-jetbrains text-[10px] text-[var(--accent)] tracking-[0.12em] mt-0.5 shrink-0">{s.n}</span>
                  <div>
          ...
          {sent ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className={labelCls}>{lang === "en" ? "Full name *" : "Nom complet *"}</label>
                  <input type="text" value={form.name} onChange={set("name")} placeholder="Jane Dupont" className={inputCls} />
                </div>
                <div>
                  <label className={labelCls}>{lang === "en" ? "Email *" : "Email *"}</label>
                  <input type="email" value={form.email} onChange={set("email")} placeholder="jane@gallery.com" className={inputCls} />
                </div>
              </div>

              <div>
                <label className={labelCls}>{lang === "en" ? "Organization / Institution" : "Organisation / Institution"}</label>
                <input type="text" value={form.organization} onChange={set("organization")} placeholder={lang === "en" ? "Gallery name, museum, foundation…" : "Nom de la galerie, musée, fondation…"} className={inputCls} />
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className={labelCls}>{lang === "en" ? "Purpose" : "Objectif"}</label>
                  <select value={form.purpose} onChange={set("purpose")} className={`${inputCls} cursor-pointer`}>
                    <option value="">{lang === "en" ? "Select…" : "Choisir…"}</option>
                    {PURPOSES[lang].map((p) => <option key={p} value={p}>{p}</option>)}
                  </select>
                </div>
                <div>
                  <label className={labelCls}>{lang === "en" ? "Budget range" : "Budget indicatif"}</label>
                  <select value={form.budget} onChange={set("budget")} className={`${inputCls} cursor-pointer`}>
                    <option value="">{lang === "en" ? "Select…" : "Choisir…"}</option>
                    {BUDGETS[lang].map((b) => <option key={b} value={b}>{b}</option>)}
                  </select>
                </div>
              </div>

              <div>
                <label className={labelCls}>{lang === "en" ? "Your brief *" : "Votre brief *"}</label>
                <textarea
                  value={form.message}
              ...
              {error && (
                <p className="text-red-400 text-xs border border-red-900/40 bg-red-950/30 px-4 py-3">{error}</p>
              )}
