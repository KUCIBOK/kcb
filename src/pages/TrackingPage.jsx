...
function StatusIcon({ status, className = 'w-6 h-6' }) {
  switch (status) {
    ...
    case 'in_transit':
      return <Truck className={`${className} text-kcb-or`} />
    case 'customs_cleared':
      return <ShieldCheck className={`${className} text-kcb-or`} />
    case 'in_preparation':
      return <Package className={`${className} text-yellow-400`} />
    case 'rejected':
      return <AlertCircle className={`${className} text-red-400`} />
    default:
      return <Clock className={`${className} text-kcb-pierre`} />
  }
}
...
export default function TrackingPage() {
  ...
  useEffect(() => {
    if (!trackingId) {
      setLoading(false)
      setErrorMsg('Identifiant de suivi manquant.')
      return
    }
      ...
      <div className="max-w-2xl mx-auto">
        ...
        {!loading && errorMsg && (
          <div className="bg-kcb-ardoise border border-red-700/40 rounded-[4px] p-8 text-center">
            ...
            <Link
              to="/"
              className="inline-block mt-6 text-sm text-kcb-or hover:text-kcb-or transition"
            >
          ...
          <div className="space-y-6">
            ...
            <RevealOnScroll delay={0.1}>
              <div className="bg-kcb-ardoise border border-white/[0.06] rounded-[4px] p-6">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`text-xs font-medium px-3 py-1.5 rounded-full border ${STATUS_COLORS[delivery.status] || 'text-kcb-pierre border-white/[0.08]'}`}
                  >
                    {delivery.status}
                  </span>
                </div>
            ...
            <RevealOnScroll delay={0.2}>
              <div className="bg-kcb-ardoise border border-white/[0.06] rounded-[4px] p-5">
                ...
                {/* Checklist emballage */}
                {delivery.packagingChecklist &&
                  Object.values(delivery.packagingChecklist).some(Boolean) && (
                    <div className="mt-4 pt-4 border-t border-white/[0.06]">
                      <p className="text-kcb-pierre text-xs uppercase tracking-wider mb-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {[
                          ...
                          .filter((item) => delivery.packagingChecklist[item.key])
                          .map((item) => (
                            <span
                              key={item.key}
                              className="flex items-center gap-1 text-xs bg-green-900/30 text-green-400 border border-green-800/50 rounded-full px-2 py-0.5"
                            >
