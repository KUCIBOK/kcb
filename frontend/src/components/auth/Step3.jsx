...
import { MetamaskSignUp } from "../../api/useAuth";
...
export function Step3({ formState, setFormState }) {
    ...
    const handleNext = async () => {
        if(formState.role != '' && formState.role != null){
            if(formState.connectMethod == "email"){
                setFormState({...formState, step : 3})
            }
            if(formState.connectMethod == "metamask"){
                setFormState({...formState, loading : true})
                const {address, signature, message} = formState.credentials
                if(address && signature && message){
                    const {user, artist, profile, error} = await MetamaskSignUp({
                        address : address,
                        signature : signature,
                        message : message,
                        role : formState?.role
            ...
            <div className="bg-card rounded-xl border border-gray-800 shadow-sm p-6">
              ...
              <div className="mt-6 text-center">
                <button
                  onClick={() => {
                    if (formState.connectMethod === 'email') {
                      setFormState({ ...formState, error: '', step: 1, loading: false });
                    }
                    if (formState.connectMethod === 'metamask') {
                      setFormState({ ...formState, error: '', step: 0, loading: false });
                    }
                  }}
