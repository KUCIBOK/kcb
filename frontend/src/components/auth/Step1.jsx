...
export const Step1 = ({ formState, setFormState, onGoogleSignup }) => {
    ...
    const handleConnectMetamask = async () => {
        ...
        if (!(await checkMetaMask())) {
            setFormState({ ...formState, error: "Vous ne disposez pas de l'extension Metamask. Veuillez l'installer pour continuer.", loading: false });
            return;
        }
        try {
            const credentials = await getMetamaskAddress();
            if (credentials?.address) {
                setFormState({ ...formState, error: "", credentials, step: 2, connectMethod: 'metamask' });
                return;
            }
            setFormState({ ...formState, error: "Erreur pendant l'authentification Metamask (Veuillez réessayer)", loading: false });
        } catch {
